...
from System_CHP_g import run_model
...
def solve_MESP_brent(input_data, low=0.03, high=5.0):
    def f(price):
        local_inputs = input_data.copy()
        local_inputs['electricity_price'] = price
        return run_model(inputs(**local_inputs))['NPV']
    
    f_low = f(low)
    f_high = f(high)

    # If NPV already >= 0 at minimum price → no subsidy needed
    if f_low >= 0:
        return low

    # If NPV still negative at max price → break-even not reachable
    if f_high <= 0:
        return np.nan
...
def main(n_sims, batch_id, base_seed=42):
    ...
    np.random.seed(seed)
    print(f"Batch {batch_id}: seed={seed}, sims={n_sims}")
    ...
    param_pert_ranges = {
        ...
        'CHP_efficiency': (0.66, 0.685, 0.71),
        'CHP_elec_efficiency': (0.24, 0.35, 0.36),
        #Manure properties
    ...
    # sampling
    samples = {k: pert_sample(*v, size=n_sims) for k, v in param_pert_ranges.items()}
    ...
    for i in range(n_sims):
        try:
            inputs_dict = {k: samples[k][i] for k in samples}
            data = inputs(**inputs_dict)
            r = run_model(data)
            mesp = solve_MESP_brent(inputs_dict)
            grant = solve_grant_brent(inputs_dict)
            results.append({**inputs_dict, **r, 'MESP': mesp, 'Minimum_Grant_percentage': grant})
        except Exception as e:
            print(f"Simulation {i+1} failed: {e}")

    # save CSV
    df = pd.DataFrame(results)
    out_file = f"data/uncertainty_chp_batch_{batch_id}.csv"
