...
def create_system(data, manure_storage = True, grass_storage = True, digestate_storage = False, heat_mixtank = False, parlor_system = True, upgrading = False):
    ...
    from Units_g import ManureStorageTank, GrassStorageTank, Shredder, MixingTank, ContinousFermentation, H2SRemovalUnit, Biogas_cooling, SolidsSeparator, DigestateStorageTank, Membrane_Separation, BoilerUnit, CHPUnit
    ...
    CHP = CHPUnit(ID='CHP', ins=(H2S_Removal-0, 'Water'), 
                  outs=('Emissions', 'Steam'), 
                  CHP_efficiency=data['CHP_efficiency'], 
                  CHP_elec_efficiency=data['CHP_elec_efficiency'],
    ...
    system_sys = bst.System('RNG_plat', path = (Manure_Pit, Manure_Pump, Grass_Storage, Grass_Shredder, Mixing_Tank, Digester, Moisture_Removal, Water_trap, H2S_Removal, Dewater, Recycle_loop, Digestate_Pump, Mixing_Tank, Digestate_storage, CHP))

    return system_sys, prices, manure_data, grass_data, water_data, Carbon_Nitrogen_ratio, Digester


def run_model(data):
    system_sys, prices, manure_data, grass_data, water_data, Carbon_Nitrogen_ratio, Digester = create_system(data)
    system_sys.simulate()
    ...
    tea = AD_TEA(system                 =   system_sys,
                ...
                upgrading_cost          =   False,                      #Upgrading cost, if any
    ...
    #system_sys.save_report('System_CHP_SWG_CN.xlsx') #This is to save a report with all the information of the system, including stream tables, unit tables, and economic tables. It can be useful for debugging and for understanding the system better.

    return {'NPV': npv, 'IRR': irr, 'TCI': tci, 'CN_ratio': Carbon_Nitrogen_ratio, 
            'biogas_kg_hr': biogas_flow,
            'Area_grass_ha': grass_data['area_grass'], 
            'Grass_flow_kg_hr': grass_data['flowrate'],
            'Grass_annual_flowrate_kg_yr': grass_data['flow_per_year'],
